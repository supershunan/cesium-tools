...
export default function Voxel({ viewer }: { viewer: Cesium.Viewer }) {
    ...
    const setupMouseMoveHandler = useCallback((
        ...
        // 移除之前的事件处理器
        const viewerWithHandler = viewer as Cesium.Viewer & { _dataGridHandler?: Cesium.ScreenSpaceEventHandler };
        ...
        handler.setInputAction((click: { position: Cesium.Cartesian2 }) => {
            ...
            if (!cartesian) {
                ...
                const cartesianEllipsoid = viewer.camera.pickEllipsoid(click.position, viewer.scene.globe.ellipsoid);
                ...
                if (
                    ...
                    console.log('周围数据值(pickEllipsoid):', {
                        ...
                        left: xIndex > 0 ? `[${yIndex}][${xIndex - 1}] = ${gridData[yIndex]?.[xIndex - 1]}` : 'N/A',
                        right: xIndex < header.xSize - 1 ? `[${yIndex}][${xIndex + 1}] = ${gridData[yIndex]?.[xIndex + 1]}` : 'N/A',
                        up: yIndex > 0 ? `[${yIndex - 1}][${xIndex}] = ${gridData[yIndex - 1]?.[xIndex]}` : 'N/A',
                        down: yIndex < header.ySize - 1 ? `[${yIndex + 1}][${xIndex}] = ${gridData[yIndex + 1]?.[xIndex]}` : 'N/A'
                    });

                    // eslint-disable-next-line no-console
                    console.log(`经纬度: (${longitude.toFixed(6)}, ${latitude.toFixed(6)}), 索引: [${xIndex}, ${yIndex}], 原始值: ${originalValue}, 数组长度: [${gridData.length}, ${gridData[0]?.length}]`);
                } else {
                    // eslint-disable-next-line no-console
                    console.log('不在数据区域内', {
    ...
    const renderShader = useCallback((data: GridData) => {
        ...
        if (!canvasRef.current || canvasRef.current.width !== xSize || canvasRef.current.height !== ySize) {
        ...
        if (materialRef.current && primitiveRef.current) {
            ...
            // 更新颜色规则uniform
            for (let i = 0; i < colorRuleCount; i++) {
                materialUniforms[`colorRuleMax${i}`] = colorRules[i].max === Infinity ? 999999 : colorRules[i].max;
    ...
    // 导出更新数据的方法，供外部调用
    const updateData = useCallback((data: GridData) => {
        if (viewer) {
            renderShader(data);
        }
    }, [viewer, renderShader]);
    ...
    useEffect(() => {
        ...
        setTimeout(() => {
            ...
            setInterval(() => {
                getNcData('http://222.74.18.86:7085/fxtraincold/' + dataURL[index])
                .then((data) => {
                    // eslint-disable-next-line no-console
                    console.log('数据加载完成', data);
    ...
    const getNcData = async (url?: string): Promise<GridData> => {
        ...
        try {
            const res = await fetch(url ? url : '/public/resources/SX002_20250809190000_CR.zip', {
                method: 'GET',
                headers: {
